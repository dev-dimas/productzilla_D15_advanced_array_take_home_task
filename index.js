...
let transactionLogs,
  ...
  incomeEachTransaction,
  transactionByQuantity,
  storeLocationRating,
  incomeEachLocation = [];

const setTransactionLogs = () => {
  transactionLogs = data.map((log) => {
    return {
      idTransaction: log.id,
      storeLocation: log.storeLocation,
      userEmail: log.customer.email,
      purchaseMethod: log.purchaseMethod,
...
const setTransactionByLocation = () => {
  ...
  data.forEach((log) => {
    const { storeLocation, ...restData } = log;
    const locExist = result.find((log) => log.storeLocation === storeLocation);
    if (!locExist) {
      result.push({
        storeLocation,
        transactionLogs: [],
    ...
    const index = result.findIndex((logs) => logs.storeLocation === storeLocation);
    index !== -1 && result[index].transactionLogs.push(restData);
  });

  transactionByLocation = result;
};

const setIncomeEachTransaction = () => {
  !transactionLogs && setTransactionLogs();

  incomeEachTransaction = transactionLogs.map((log) => {
    let totalIncome = 0;
    log.items.forEach((item) => {
      totalIncome += item.price.$numberDecimal * item.quantity.$numberInt;
    });
    return {
...
const setTransactionByQuantity = () => {
  !transactionLogs && setTransactionLogs();

  transactionByQuantity = transactionLogs.map((log) => {
    let totalQuantity = 0;
    log.items.forEach((item) => {
      totalQuantity += parseInt(item.quantity.$numberInt);
    });
    return {
      idTransaction: log.idTransaction,
      totalQuantity,
...
const setStoreLocationRating = () => {
  !transactionByLocation && setTransactionByLocation();

  storeLocationRating = transactionByLocation.map((log) => {
    const { storeLocation, transactionLogs } = log;
    let totalRating = [];

    transactionLogs.forEach((transaction) => {
      const { satisfaction } = transaction.customer;
      totalRating.push(parseInt(satisfaction.$numberInt));
    });

    return {
      storeLocation,
      ratings: transactionLogs.map((transaction) => {
        const { email, satisfaction } = transaction.customer;
        return {
          customer: {
            email,
            satisfaction: parseInt(satisfaction.$numberInt),
          },
        };
      }),
      averageRating: +(
        totalRating.reduce((total, rating) => {
          return total + rating;
        }) / totalRating.length
      ).toFixed(2),
    };
...
const setIncomeEachLocation = () => {
  !transactionByLocation && setTransactionByLocation();

  incomeEachLocation = transactionByLocation.map((log) => {
    let totalIncome = 0;
    log.transactionLogs.forEach((sale) => {
      sale.items.forEach((item) => {
        totalIncome += item.quantity.$numberInt * item.price.$numberDecimal;
      });
    });
    return {
      storeLocation: log.storeLocation,
...
setTransactionLogs();
setTransactionByLocation();
setIncomeEachTransaction();
setTransactionByQuantity();
setStoreLocationRating();
setIncomeEachLocation();

console.log('=== Array baru dari array data ===');
console.log(transactionLogs);
console.log('\n=== Data transaksi berdasakan lokasi pembelian ===');
console.log(transactionByLocation);
console.log('\n=== Jumlah keuntungan dari setiap transaksi ===');
console.log(incomeEachTransaction);
console.log('\n=== Jumlah barang yang dibeli dari setiap transaksi ===');
console.log(transactionByQuantity);
console.log('\n=== Rating setiap toko ===');
console.log(storeLocationRating);
console.log('\n=== Jumlah keuntungan dari setiap toko ===');
console.log(incomeEachLocation);
